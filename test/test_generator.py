...
from core.restclient.generator.generate_client_methods import Application, generate_python_module
...
class GeneratorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.curl.setopt(pycurl.HTTPHEADER, ["Accept:application/vnd.sun.wadl+xml", 
                                             "Authorization:Basic %s" % base64.encodestring('%s:%s' % (self.conf.get(section='main', option='username'), self.conf.get(section='main', option='password'))).replace('\n', '')])
        ...
        self.generatorMethodsPath = os.path.join(os.path.join(self.generatorPath, self.conf.get(section='apiversion', option='version').replace('.', '_')), 'methods.py')
