...
The following options are supported:
    ...
    -v <version>        Used to locate a subdirectory in which to write the
                        output code.   
...
try:
    import dedup_map.duplicateMethods
...
def promptResolveMethodNameConflict(methodentry, othermethodentry):
    ...
    while True:
        method_alias = input('Enter an alias for method (1): [%s] ' % methodentry['name'])
        ...
        other_method_alias = input('Enter an alias for method (2): [%s] '%othermethodentry['name'])
...
def generate_python_method(method):
    ...
    for param in method.all_params :
        if not param.format().startswith("self") :
            param_names.append(param.format())  
...
def generate_python_class(cls):
    ...
    if cls.name.lower() == "global" :
        className = cls.name.lower() + "s(Endpoint)"
    else :
        className = cls.name.lower() + "(Endpoint)"
    
    return PYTHON_CLASS_TEMPLATE % { "class_name": className,
                                     "class_docs": docs,
                                     "methods": methods }
