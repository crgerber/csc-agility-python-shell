...
with_resources = filter(lambda c: getattr(c, 'resources', None), compute)
...
report = [(id, len(disks)) for id, disks in computeDisks.items()]
for entry in report: print entry
# <demo> --- stop ---
#Or any kind of reports actually
report = ['compute: [%s], name: %s, state: %s: disks: %s'%(id, computeMap[id].name, computeMap[id].state, ['%s : %s %s'%(d.name, d.quantity, d.units) for d in disks]) for id, disks in computeDisks.items()]
for entry in report: print entry