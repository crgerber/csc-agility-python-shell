...
class MessageTest (unittest.TestCase):
    ...
    def test_1_encode(self):
        ...
        self.assertEqual(str(msg), self.__a)
        ...
        self.assertEqual(str(msg), self.__b)
        ...
        self.assertEqual(str(msg), self.__c)

    def test_2_decode(self):
        msg = Message(self.__a)
        self.assertEqual(msg.get_int(), 23)
        self.assertEqual(msg.get_int(), 123789456)
        self.assertEqual(msg.get_string(), 'q')
        self.assertEqual(msg.get_string(), 'hello')
        self.assertEqual(msg.get_string(), 'x' * 1000)

        msg = Message(self.__b)
        self.assertEqual(msg.get_boolean(), True)
        self.assertEqual(msg.get_boolean(), False)
        self.assertEqual(msg.get_byte(), '\xf3')
        self.assertEqual(msg.get_bytes(2), '\x00\x3f')
    ...
    def test_3_add(self):
        ...
        msg.add('cat')
        msg.add(['a', 'b'])
        self.assertEqual(str(msg), self.__d)

    def test_4_misc(self):
        msg = Message(self.__d)
        self.assertEqual(msg.get_int(), 5)
        self.assertEqual(msg.get_mpint(), 0x1122334455)
        self.assertEqual(msg.get_so_far(), self.__d[:13])
        self.assertEqual(msg.get_remainder(), self.__d[13:])
        msg.rewind()
        self.assertEqual(msg.get_int(), 5)
